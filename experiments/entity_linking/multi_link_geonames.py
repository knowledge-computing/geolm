...
sys.path.insert(0,'/home/zekun/joint_model/src/')
from dataset_utils.dataset_loader import SpatialDataset
...
def get_nl_feature(text, gt_lat, gt_lon, start_span, end_span, model, model_name, tokenizer, spatial_dist_fill, device):
    ...
    if model_name == 'joint-base' or model_name=='joint-large' or model_name == 'simcse-base':
        nl_outputs = model(torch.tensor([nl_tokens['input_ids']]).to(device), 
                ...
                spatial_position_list_x = nl_tokens['norm_lng_list'], 
                spatial_position_list_y = nl_tokens['norm_lat_list'] , 
...
def get_geoname_features(geonames_cand, model, model_name, tokenizer, spatial_dist_fill, device, spatial_dataset, batch_size):
    ...
    # if len(geonames_cand) <= batch_size:
    #     # process all cands at once 
    # else:
    #     # break into rounds

    # break down into chunks
    for chunk_idx in range(0, int(math.ceil(1.0 * len(geonames_cand) / batch_size ))):

        pseudo_sent_concat = []
        attention_mask_concat= []
        sent_position_id_concat = []
        pivot_token_idx_concat = []
        position_x_concat = []
        position_y_concat = []

        for gn_cand in geonames_cand[chunk_idx * batch_size: (chunk_idx+1)*batch_size]:
            ...
            pseudo_sent_concat.append(geo_data['pseudo_sentence'])
            attention_mask_concat.append(geo_data['attention_mask'])
            sent_position_id_concat.append(geo_data['sent_position_ids'])
            pivot_token_idx_concat.append(geo_data['pivot_token_idx'])
            position_x_concat.append(geo_data['norm_lng_list'])
            position_y_concat.append(geo_data['norm_lat_list'])
            n_samples = len(pseudo_sent_concat)

        
        if model_name == 'joint-base' or model_name=='joint-large' or model_name == 'simcse-base':
            geo_outputs = model(torch.stack(pseudo_sent_concat, dim = 0).to(device),
                            attention_mask = torch.stack(attention_mask_concat, dim = 0).to(device), 
                            sent_position_ids = torch.stack(sent_position_id_concat, dim = 0).to(device),
                            pivot_token_idx_list= torch.stack(pivot_token_idx_concat, dim = 0).to(device),
                            spatial_position_list_x = torch.stack(position_x_concat, dim = 0).to(device),
                            spatial_position_list_y = torch.stack(position_y_concat, dim = 0).to(device),
                            token_type_ids = torch.ones( n_samples, len(geo_data['pseudo_sentence'])).int() .to(device), # type 1 for geo data 
                            )
            geo_feat = geo_outputs.pooler_output
            geo_feat = geo_feat.detach().cpu().numpy()
            ...
            # geo_outputs = model(torch.stack(pseudo_sent_concat, dim = 0).to(device),
            #                 attention_mask = torch.stack(attention_mask_concat, dim = 0).to(device), 
            #                 token_type_ids = torch.ones( n_samples, len(geo_data['pseudo_sentence'])).int() .to(device), 
            #                 )
    ...
    geo_feat_list = np.concatenate(geo_feat_list, axis = 0)
        
    return geo_feat_list, geo_loc_list, geo_id_list


def wiktor_linking(out_path, query_data, geonames_dict, model, model_name, tokenizer, spatial_dist_fill, device, spatial_dataset, batch_size):
    ...
    for query_name, paragraph_list in query_data.items():
        ...
        if query_name in geonames_dict:
            ...
            geoname_features, geonames_loc_list, geonames_id_list = get_geoname_features(geonames_cand, model, model_name, tokenizer, spatial_dist_fill, device, spatial_dataset, batch_size)
        else:
            continue 
        ...
        # 'spans':spans}
            ...
            ret_dict['sorted_match_geoname_id'] = ret_dict['sorted_match_geoname_id'][:20]
            ret_dict['closest_match_geonames_loc'] = ret_dict['closest_match_geonames_loc'][:20]
...
def toponym_linking(out_path, query_data, geonames_dict, model, model_name, tokenizer, spatial_dist_fill, device, spatial_dataset, batch_size):
...
def entity_linking_func(args):
    ...
    if 'WikToR' in args.query_dataset_path:
        ...
        eval_info = wiktor_linking(out_path, query_data, geonames_dict, model, model_name, tokenizer, spatial_dist_fill, device, spatial_dataset, args.batch_size)
        ...
        eval_info = toponym_linking(out_path, query_data, geonames_dict, model, model_name, tokenizer, spatial_dist_fill, device, spatial_dataset, args.batch_size)
...
def main():
    ...
    # CUDA_VISIBLE_DEVICES='1' python3 multi_link_geonames.py --model_name='joint-base' --distance_norm_factor=100 --spatial_dist_fill=900 --spatial_bert_weight_dir='/home/zekun/weights_base_0511/' --spatial_bert_weight_name='ep4_iter08000_0.0326.pth' --out_dir='debug' --batch_size=24
