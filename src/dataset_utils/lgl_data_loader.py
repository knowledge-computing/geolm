...
class ToponymDataset(SpatialDataset):
    ...
    def load_data(self, index):
        ...
        labels = np.array([self.label_to_id['O'] for i in range(len(input_ids))])
        ...
        for toponym in toponyms:
            ...
            if start < 0 or end < 0: 
                continue # skip wrong annotations in GWN

            if start not in offset_mapping_dict_start:
        ...
        if len(input_ids) > self.max_token_len: 
            rand_start = np.random.randint(1, len(input_ids) - self.max_token_len +1) # Do not include CLS and SEP [inclusive, exclusive)
        ...
        ret_dict['norm_lng_list'] = torch.tensor([0 for i in range(self.max_token_len)]).to(torch.float32)
        ret_dict['norm_lat_list'] = torch.tensor([0 for i in range(self.max_token_len)]).to(torch.float32)
